...
**Stage 1 done and verified** (2026-07-24) — Minimum Viable ARGUS is complete and runnable end-to-end.
`data/{acme_corp,globex}/{filings,prices,news}.json` holds 5 years of financials, a 12-month price
series, and headlines for each (Globex's 2025-01-22 headline is a deliberately misleading claim, seeded
for the Stage 3/4 verifier and reconciler to catch later — not caught yet, which is expected). Built:
`filings_agent`, `market_agent`, `sentiment_agent` (parallel, unique `output_key`s, verbatim-JSON
instructions) → `quant_agent` (real code execution) → `synthesis_agent`, composed as a `SequentialAgent`
root_agent in `agent.py`. Verified live in `adk web`: full pipeline on Globex produced a drafted thesis
whose every number traces to the Quant agent's executed pandas code, hand-checked correct (CAGR 7.52%,
margin -4.02%, debt +$650M, etc.); a negative test on a nonexistent company ("Initech") degraded
gracefully through all 5 agents with no crash and no invented numbers. Still no git repo — this is the
"4 agents done" milestone the user set as the trigger to revisit git; ask before running `git init`.

Two things to circle back on later: (1) `MODEL_FLASH` is temporarily `gemini-3.5-flash-lite`, not the
usual `-latest` alias — see the Models note above and `config.py`; switch back once iteration slows down.
(2) The Dev UI trace shows recurring "Performance" warning icons on most events — not investigated, no
observed functional impact so far (all outputs have been correct), worth a quick look if anything actually
breaks.

Next: **Stage 2** — self-critique `LoopAgent` (Red-Team Critic + Refiner, early-exit on PASS via
`escalate=True`), inserted after Synthesis.

## How to help me

- Go stage by stage; finish and verify one before starting the next.
- After each stage: run in `adk web`, do a numeric hand-check + a negative test. Hold off on git commits
  until told otherwise (the "Stage 1 done" trigger has now been reached — confirm with the user before
  the first `git init`, don't just do it).
...
- **Verify framework facts before trusting old docs/memory** — ADK and Gemini model IDs both drifted
  ...
  ai.google.dev before pinning any new class name or model ID in a later stage. Stage 1 found two more
  API gotchas the hard way — see non-negotiable principles 7-9 — so keep verifying rather than assuming.
- **Watch the free-tier quota.** Fan-out architectures burn several raw API calls per user turn. If a
  run throws `ExceptionGroup` / `RESOURCE_EXHAUSTED` in the server logs, that's the quota, not a bug —
  check `preview_logs` before debugging application code.
